...
Proof.
    intros.
    assert (exists x, Bet C x C /\ Bet B x D) by (eapply inner_pasch;eBetween).
    ex_and H1 x.
    Between.
Qed.
...
Proof.
    ...
    assert (A<>B).
      ...
      subst; tauto.
    assert (Cong C A C' A') by (apply (five_segments _ _ _ _ _ _ _ _ H1 );auto using cong_trivial_identity, cong_commutativity).
    apply H3; Cong.
Qed.
