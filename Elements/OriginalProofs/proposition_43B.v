...
let Tf:=fresh in
assert (Tf:exists e, (BetS B A e /\ Cong A e B A)) by (conclude postulate_extension);destruct Tf as [e];spliter.
...
let Tf:=fresh in
assert (Tf:exists c, (BetS B C c /\ Cong C c B C)) by (conclude postulate_extension);destruct Tf as [c];spliter.
