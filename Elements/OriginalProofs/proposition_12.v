...
let Tf:=fresh in
assert (Tf:exists E, (BetS C B E /\ Cong B E C B)) by (conclude postulate_extension);destruct Tf as [E];spliter.
...
let Tf:=fresh in
assert (Tf:exists F, (BetS E C F /\ Cong C F E C)) by (conclude postulate_extension);destruct Tf as [F];spliter.
...
let Tf:=fresh in
assert (Tf:exists K, CI K C C E) by (conclude circle);destruct Tf as [K];spliter.
assert (Cong C B C B) by (conclude cn_congruencereflexive).
assert (Lt C B C E) by (conclude_def Lt ).
assert ((CI K C C E /\ InCirc B K)) by (conclude inside).
let Tf:=fresh in
assert (Tf:exists P Q, (Col A B P /\ Col A B Q /\ OnCirc P K /\ OnCirc Q K /\ BetS P B Q)) by (conclude postulate_line_circle);destruct Tf as [P[Q]];spliter.
assert (Cong C P C E) by (conclude on).
assert (Cong C Q C E) by (conclude on).
...
assert (Col P Q M) by (conclude_def Col ).
assert (eq C C) by (conclude cn_equalityreflexive).
assert (Col C M C) by (conclude_def Col ).
