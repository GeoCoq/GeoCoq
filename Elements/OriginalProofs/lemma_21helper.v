...
let Tf:=fresh in
assert (Tf:exists F, (BetS B A F /\ Cong A F A C)) by (conclude postulate_extension);destruct Tf as [F];spliter.
assert (neq E C) by (forward_using lemma_betweennotequal).
let Tf:=fresh in
assert (Tf:exists G, (BetS B E G /\ Cong E G E C)) by (conclude postulate_extension);destruct Tf as [G];spliter.
