...
let Tf:=fresh in
assert (Tf:exists E, (BetS A C E /\ Cong C E A D)) by (conclude postulate_extension);destruct Tf as [E];spliter.
...
let Tf:=fresh in
assert (Tf:exists F, (BetS A D F /\ Cong D F A C)) by (conclude postulate_extension);destruct Tf as [F];spliter.
