...
Proof.
  ...
  apply grada_stab with D E F.
    ...
    apply (conga2_isi__isi D E F A B C); trivial.
...
Proof.
  ...
  { rename H into HConga; rename D0 into G; rename E0 into H; rename F0 into I.
    intros K L M HSuma.
    suma.assert_diffs.
    apply (conga2_isi__isi D E F G H I D E F A B C) in HIsi; CongA.
    apply (conga3_suma__suma D E F G H I K L M D E F A B C K L M) in HSuma; CongA.
    apply grada_stab with D E F; trivial.
  }
  assert (Hd1 := isi_distincts D0 E0 F0 A B C H0); assert (Hd2 := isi_distincts D E F G H I HIsi); spliter.
  destruct (ex_suma D E F D0 E0 F0) as [K [L [M HSuma]]]; auto.
  intros K0 L0 M0 HSuma2.
  assert (HIsi2 : Isi D E F D0 E0 F0).
    apply isi_lea2__isi with D E F G H I; Lea.
    apply isi_suma__lea123789 with A B C; trivial.
  apply grada_stab with K L M.
    apply IHHGA2; trivial.
    apply isi_assoc_2 with D E F D0 E0 F0 G H I; trivial.
...
Proof.
  ...
  assert (HIsi : Isi P Q R A0 O A1).
  { apply isi_lea2__isi with A0 O B A0 O B.
    - apply acute__isi, l11_43_aux; Col.
  ...
  { intro HCol; apply HNCol1.
    ...
    apply isi_suma__out546 in HSuma2; Col.
    apply (conga2_isi__isi P Q R A0 O A1); CongA.
  }
  ...
  { apply col_two_sides_bet with O.
    ...
    apply lea_in_angle.
      apply (l11_30 P Q R P' Q' R'); CongA; apply isi_suma__lea123789 with A0 O A1; trivial.
  ...
  { assert_diffs.
    ...
    apply isi2_suma2__conga456 with P Q R P' Q' R'; trivial.
    - apply (conga2_isi__isi A0 O A' A' O A''); CongA.
      split; auto; split.
        right; intro; Col.
  ...
  exists A''; repeat (split; trivial).
    ...
    apply (cong_preserves_le A0 A1 A' A''); Cong.
...
Proof.
  ...
  induction HGA2.
    ...
    intro HIsi; apply HNIsi, (conga2_isi__isi D E F A B C); CongA.
  intro HNIsi.
  elim (isi_dec D E F D0 E0 F0); [|apply IHHGA2; trivial].
  intro HIsi; clear IHHGA2.
  assert (Hd := isi_distincts D E F D0 E0 F0 HIsi); spliter.
  destruct (ex_suma D E F D0 E0 F0) as [P [Q [R HSuma]]]; auto.
  exists P; exists Q; exists R; split.
    apply grada2_isi_suma__grada with D E F D0 E0 F0; trivial.
...
Proof.
  ...
  { apply nbet_isi_suma__acute with D E F; trivial.
  ...
  assert_diffs.
  destruct (isi_dec P1 Q1 R1 P1 Q1 R1) as [HIsi|HNIsi].
  { destruct (ex_suma P1 Q1 R1 P1 Q1 R1) as [P [Q [R HSuma1]]]; auto.
    exists P; exists Q; exists R; split.
      apply grada2_isi_suma__grada with P1 Q1 R1 P1 Q1 R1; trivial.
    left; apply isi_lea2_suma2__lea with D E F1 D E F1 P1 Q1 R1 P1 Q1 R1; trivial.
  }
  destruct (angles_archi_aux A B C P1 Q1 R1 P1 Q1 R1) as [P [Q [R [HGA1 HNisi1]]]]; trivial.
...
Proof.
  ...
  elim (isi_dec P1 Q1 R1 A B C); [|intro; exists P1; exists Q1; exists R1; auto].
  intro HIsi.
  ...
  apply isi_lea2_suma2__lea with A0 B C A B C P1 Q1 R1 A B C; Lea.
  exists A; repeat (split; CongA).
...
Proof.
  ...
  destruct (isi_dec P Q R P Q R) as [HIsi|HNIsi].
  { destruct HUn as [Habs|HLea].
      absurd (Isi P Q R P Q R); trivial; apply obtuse__nisi, Habs.
    destruct (ex_suma P Q R P Q R) as [S [T [U HSuma]]]; auto.
    exists S; exists T; exists U; split.
      apply gradaexp_stab with P Q R; trivial.
    right; apply isi_lea2_suma2__lea with D E F A B C P Q R P Q R; trivial.
    apply grada__lea, gradaexp__grada, HGAE.
  }
  apply nisi__obtuse in HNIsi; auto.
  exists P; exists Q; exists R; split; auto.
Qed.
...
Proof.
  ...
  apply isi_lea2__isi with P Q R P Q R; trivial.
  apply grada__lea, gradaexp__grada, HGAE.
Qed.

End Angle_archimedes.