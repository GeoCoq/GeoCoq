...
Lemma perp_in_perp_in_col : forall A B A' B' X Y P, Perp_at P A B X Y -> Perp_at P A' B' X Y  -> Col A B A'.
Proof.
intros.
assert(HP1:= H).
assert(HP2:=H0).
...
Lemma is_image_perp_in : forall A A' X Y, A <> A' -> X <> Y -> Reflect A A' X Y -> exists P, Perp_at P A A' X Y.
Proof.
intros.
unfold Reflect in H.
induction H1.
